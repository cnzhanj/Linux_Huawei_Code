...
void Cal::setMinDisPathRecordForCar()
{
	for (int i = 0; i < mCar.size(); i++)
	{
		if (!mCar[i].getPreSet())
		{
			...
			for (int j = 0; j < mMinDisPathRecord[indexFrom][indexTo].size() - 1; j++)
			{
				//actualCross.push_back(mCross[mMinDisPathRecord[indexFrom][indexTo][j]].getId());
				int From = mCross[mMinDisPathRecord[indexFrom][indexTo][j]].getId();
				int To = mCross[mMinDisPathRecord[indexFrom][indexTo][j + 1]].getId();
...
void Cal::writeAns(string pathAnswer)
{
	...
	for (int i = 0; i < mCar.size(); i++)
	{
		if (!mCar[i].getPreSet())
		{
			...
			if (maxTime < mCar[i].getActualTime())
				maxTime = mCar[i].getActualTime();
